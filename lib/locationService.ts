...
export class LocationService {
  ...
  async getCurrentLocation(options?: {
    ...
    if (useCache && this.lastKnownLocation) {
      ...
      if (age < 5 * 60 * 1000) { // 5 minutes
        console.log('[LocationService] Using cached location, age:', Math.round(age / 1000), 'seconds');
  ...
  private async getLocationWithTimeout(timeout: number, highAccuracy: boolean): Promise<Location.LocationObject> {
  ...
  private createLocationError(error: any): LocationError {

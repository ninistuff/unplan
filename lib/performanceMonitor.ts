...
export type PerfMetadata = Readonly<Record<string, PerfScalar | PerfScalar[]>>;
...
export class PerformanceMonitor {
  ...
  startTiming(name: string, metadata?: PerfMetadata): void {
  ...
  async measureAsync<T>(name: string, fn: () => Promise<T>, metadata?: PerfMetadata): Promise<T> {
  ...
  measure<T>(name: string, fn: () => T, metadata?: PerfMetadata): T {
    this.startTiming(name, metadata);
    try {
      const result = fn();
      this.endTiming(name);
  ...
  takeMemorySnapshot(): MemoryUsage {
    ...
    let memoryInfo: Partial<PerformanceMemoryInfo> = {};

    const perf =
      typeof globalThis !== "undefined" && "performance" in globalThis
        ? (globalThis.performance as Performance & { memory?: PerformanceMemoryInfo })
        : undefined;

    if (perf?.memory) {
      const { jsHeapSizeLimit, totalJSHeapSize, usedJSHeapSize } = perf.memory;
      memoryInfo = { jsHeapSizeLimit, totalJSHeapSize, usedJSHeapSize };
    }
...
export const startTiming = (name: string, metadata?: PerfMetadata) =>
  performanceMonitor.startTiming(name, metadata);

export const endTiming = (name: string) => performanceMonitor.endTiming(name);

export const measureAsync = <T>(name: string, fn: () => Promise<T>, metadata?: PerfMetadata) =>
  performanceMonitor.measureAsync(name, fn, metadata);

export const measure = <T>(name: string, fn: () => T, metadata?: PerfMetadata) =>
...
export function usePerformanceMonitor(componentName: string) {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (__DEV__) {
        console.log(
          `[Performance] ${componentName} lifetime: ${totalLifetime}ms, renders: ${renders}`,
        );
      }
    };
  }, [componentName]);

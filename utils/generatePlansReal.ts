...
function defaultDuration(category: string): number {
  switch (category) {
    case "cinema":
      return 90; // redus de la 110
    case "museum":
      return 35; // redus de la 45
    case "park":
      return 30; // redus de la 40
    case "bar":
      return 45; // redus de la 60
    case "cafe":
      return 35; // redus de la 50
    case "restaurant":
      return 40;
    default:
      return 35; // redus de la 45
...
export async function generatePlans(opts: GenerateOptions, signal?: AbortSignal): Promise<Plan[]> {
  ...
  const radiusAttempts = [
    initialRad,
    Math.round(initialRad * 1.5),
    Math.round(initialRad * 2),
    12000,
  ];
  ...
  const maxTravelShare = opts.duration <= 150 ? 0.4 : 0.3; // mai mult timp pe drum pentru durate scurte
  ...
  console.log(
    "[Generator] Config: desired stops:",
    desiredStops,
    "max travel:",
    maxTravelMin,
    "min, transport:",
  ...
  function buildItinerary(
    ...
    for (const cat of cats) {
      ...
      console.log(
        `[Itinerary] Checking ${candidate.name} (${cat}): dist=${Math.round(dist)}m, segMin=${segMin}, stopMin=${stopMin}, totalTime=${newTotalTime}/${opts.duration}`,
      );

      if (!withinSegmentLimit(transport, dist)) {
        console.log(
          `[Itinerary] ❌ ${candidate.name}: segment too long (${Math.round(dist)}m > limit)`,
        );
        continue;
      }
      ...
      if (newTotalTime > opts.duration) {
        console.log(
          `[Itinerary] ❌ ${candidate.name}: total time exceeds duration (${newTotalTime} > ${opts.duration})`,
        );
        break;
      }
      ...
      console.log(
        `[Itinerary] ✅ Added ${candidate.name}: travel=${travelMin}min, visit=${visitMin}min`,
      );
    ...
    console.log(
      `[Itinerary] Built ${results.length} stops, travel: ${travelMin}min, visit: ${visitMin}min`,
    );
    ...
    console.log(
      `[Itinerary] ✅ Final: ${results.length} stops, ${travelMin}min travel, ${visitMin}min visit`,
    );
  ...
  for (let i = 0; i < 5 && plans.length < 3; i++) {
    ...
    const p: Plan = {
      ...
      km:
        Math.round(itin.travelMin * (mode === "foot" ? 0.08 : mode === "bike" ? 0.24 : 0.3) * 10) /
        10, // aproximativ
      min: Math.min(opts.duration, itin.travelMin + itin.visitMin),
      cost: (budget === Infinity
        ? costUnknown
          ? undefined
          : cost

...
console.log(`[GeneratePlans] Distance from Bucharest center: ${Math.round(distanceFromBucharest)} meters`);
```
...
if (locationAge > 5 * 60 * 1000) { // Older than 5 minutes
  console.warn(`Location seems cached (${locationAgeMinutes} min old), trying to get fresh location...`);
...
if (distanceFromBucharest > 50000) { // More than 50km from Bucharest
  console.warn(`Location seems far from Bucharest (${Math.round(distanceFromBucharest/1000)}km), but using detected location anyway`);
}
```

...
function buildSimplePlan(id: string, title: string, start: LatLng, pool: POI[], seq: Array<POI["category"]>, transportMode: "foot" | "bike" | "driving"): Plan {
  ...
  console.log(`[FastPlan] ${nearbyPOIs.length} POIs within ${maxDistance}m from ${start.lat}, ${start.lon}`);
  ...
  console.log(`[FastPlan] Plan ${id} completed: ${steps.length} steps, ${stops.length} POIs, ${km}km, ${min}min`);
  return plan;
}
```

...
try {
  [A, B, C] = await Promise.all([
    buildUltraRealisticPlan("A", "Plan A", center, [...pois], seqA2, transportMode, dur, context, opts),
    buildUltraRealisticPlan("B", "Plan B", center, [...pois], seqB2, transportMode, dur, context, opts),
    buildUltraRealisticPlan("C", "Plan C", center, [...pois], seqC2, transportMode, dur, context, opts)
  ]);
...
console.log(`[GeneratePlans] Plan ${id}: POI pool size=${pool.length}, Categories=${seq.join(', ')}`);

// Debug: Show first few POIs to verify data
if (pool.length > 0) {
  console.log(`[GeneratePlans] Plan ${id}: Sample POIs:`, pool.slice(0, 3).map(p => `${p.name} (${p.category})`));
} else {
...
console.log(`[QualityFilter] ${category}: Using all ${filtered.length} POIs (quality filter disabled for debugging)`);
...
for (const poi of emergencyPOIs) {
  ...
  console.log(`[GeneratePlans] Plan ${id}: EMERGENCY POI: ${poi.name} (${poi.category}, ${Math.round(distance)}m)`);

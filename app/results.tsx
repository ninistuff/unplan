  ...
  const { params, appliedFallbacks, normalizedLink, seed } = useMemo(() => {
    ...
    if (bRaw === "inf") b = "inf";
      ...
      if (!Number.isFinite(bn)) {
        ...
        b = Math.max(0, Math.min(300, bn));
        if (bn !== (b as number)) fallbackUsed = true;
      }
    }
      ...
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 100 }}>
        ...
        <View style={{ alignItems: "center", marginTop: 20, marginBottom: 20 }}>
          ...
          <Text style={{ fontSize: 14, color: "#666", textAlign: "center", paddingHorizontal: 20 }}>
            {lang === "ro"
              ? "Creez planuri personalizate pentru tine..."
              : "Creating personalized plans for you..."}
          </Text>
    ...
    switch (planId) {
      case "A":
        return {
          ...
          color: "#007AFF",
          description:
            userLang === "ro" ? "Mix perfect de activități" : "Perfect mix of activities",
        };
      case "B":
        return {
          emoji: "🎉",
              ...
              <View style={{ flexDirection: "row", marginTop: 8, alignItems: "center" }}>
                {(p.steps || []).some((s: any) => s.kind === "poi") && (
                  <Link
                    href={{
                      pathname: "/plan/[id]",
                      params: { id: String(p.id ?? idx), payload: JSON.stringify(p) },
                    }}
                    asChild
                  >
                    <Pressable
